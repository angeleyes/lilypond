...
def do_one_file (in_file_name, out_file_name, lang):
	...
	x = file_name
	while 1:
		(dir, here) = os.path.split (x)
		if os.path.exists (os.path.join (dir, 'GNUmakefile')):
			break
		(menu, here_label) = one_tab  (depth, x)
		if menu:
			tabs.append (menu)
		if here_label:
	...
	rel_name = string.join (string.split (in_file_name, '/')[1:], '/')
...
def main ():
	...
	for f in files:
		...
		if lang == 'site':
			...
			o = file_lang (dir_lang (f, 'site'), lang)
		o = os.path.join (outdir, o)
		do_one_file (f, o, lang)
